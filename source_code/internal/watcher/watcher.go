...
func (w *Watcher) SnapshotCoreAuths() []*coreauth.Auth {
	...
	if cfg != nil {
		...
		// Juma session tokens -> synthesize auths
		for i := range cfg.JumaKey {
			jk := cfg.JumaKey[i]
			sessionToken := strings.TrimSpace(jk.SessionToken)
			if sessionToken == "" {
				continue
			}
			id, token := idGen.next("juma:session", sessionToken)
			attrs := map[string]string{
				"source":               fmt.Sprintf("config:juma[%s]", token),
				"session_token":        sessionToken,
			}
			if jk.WorkspaceID != "" {
				attrs["workspace_id"] = jk.WorkspaceID
			}
			if jk.VendorConnectionID != "" {
				attrs["vendor_connection_id"] = jk.VendorConnectionID
			}
